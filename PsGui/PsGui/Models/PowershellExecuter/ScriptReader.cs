    ...
    public class ScriptReader
    {
        ...
        private void SaveInputField(string inputKey, string inputDesc, string inputType)
        {
            ...
            switch (inputType)
            {
                case "password":
                {
                    // Only one password variable will be stored
                    // This makes it easier to handle username and
                    // password caching.
                    if (_scriptPasswordVariables.Count == 0)
                    {
                        _scriptPasswordVariables.Add(new PasswordArgument(inputKey, inputDesc, inputType));
                    }
                    break;
                }
                case "username":
                {
                    // Only one username variable will be stored
                    // This makes it easier to handle username and
                    // password caching.
                    if (_scriptUsernameVariables.Count == 0)
                    {
                        _scriptUsernameVariables.Add(new UsernameArgument(inputKey, inputDesc, inputType)); break;
                    }
                    break;
                }
